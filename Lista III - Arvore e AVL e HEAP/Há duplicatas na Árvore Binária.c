...
bool verificarDuplicados(ARVORE_BINARIA* arvore, int* valores, int* tamanho) {
    if (arvore == NULL) return false;

    // Verifica se o valor j� foi encontrado anteriormente
    for (int i = 0; i < *tamanho; i++) {
        if (valores[i] == arvore->item.chave) {
            return true;
        }
    }

    // Adiciona o valor ao array de valores
    valores[(*tamanho)++] = arvore->item.chave;

    // Continua a busca pela esquerda e direita
    return verificarDuplicados(arvore->esq, valores, tamanho) || verificarDuplicados(arvore->dir, valores, tamanho);
}
...
int valores[100];  // Array para armazenar as chaves dos n�s
    int tamanho = 0;   // Tamanho atual do array
    return verificarDuplicados(arvore, valores, &tamanho);
