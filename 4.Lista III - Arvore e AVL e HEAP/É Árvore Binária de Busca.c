...
// Inicializa vari�veis
    ARVORE_BINARIA *no = arvore; // Usamos a raiz da �rvore
    ARVORE_BINARIA *pilha[100]; // Pilha para simular a travessia em ordem
    int topo = -1; // Inicializa o topo da pilha
    int chaveAnterior = -2147483648; // Valor m�nimo para a compara��o (menor que INT_MIN)

    // Percorre a �rvore
    while (no != NULL || topo >= 0) {
        // Avan�a para a sub�rvore esquerda
        while (no != NULL) {
            pilha[++topo] = no; // Empilha o n�
            no = no->esq; // Avan�a para a esquerda
        }

        // Se a pilha n�o estiver vazia, processa o n�
        if (topo >= 0) {
            no = pilha[topo--]; // Desempilha o n�

            // Verifica a propriedade da BST
            if (no->item.chave <= chaveAnterior) {
                return false; // N�o � uma �rvore bin�ria de busca
            }
            chaveAnterior = no->item.chave; // Atualiza a chave anterior

            // Avan�a para a sub�rvore direita
            no = no->dir;
        }
    }
    return true; // Se n�o houve problemas, � uma BST
